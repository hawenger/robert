    ...
    return (
        <div style={{display:'flex'}}>
                    ...
                    <br></br>
                        <h4>TONE.js</h4>
                    <br></br>
                    <p>For the generative art portion of  Robert, I experimented minimally with some of what P.5 had to offer before settling on ToneJS.  Tone is a web audio framework built on top of the Web Audio API.  This seemed like a good starting place as my first lifewas as a bassoonist and I had years of unused music education just waiting to be tapped.  Or so I thought.  Tone.js ended upbeing (and still is) a very fun but steep learning curve as classical textbooks don't have sections on "Envelopes and Oscillators" I managed to learn enough about Synths (thank you Evan), to create a fun sounding DuoSynth for the generative piece of musicthat plays in Robert's World.  Although I do not know the exact calculations, I've estimated that a user gets at least 23 minutesof music without repetition.</p>
                    <br></br>
                    <p>When it came to writing the functions that generate the music being played, I decided to stick to the good old, tried andtrue, basics of music theory.  When a song is created, a series of functions and loops are called that select a random keysignature, choose the appropriate notes for a chord in that key (root, third and sixth) and then select a common chordprogression for that key.</p>
                    <br></br>
                        <h4>RiTa</h4>
                    <p>After creating a bit of music, I realized that Robert needed to sing.  This was and is, still, my greatest challenge. Awarethat engineers have been using the Hidden Markov model for predictive speech since at least the 70s, I decided that I wanted todo something similar.  Without the time to build my own Markov chain, I settled on a general understanding of the concept andexperimented with a number of packages before settling on RiTa.</p>  
                    <br></br>
                    <p>Using RiMarkov(a function of RiTa), I fed Mr.Markov a selection of qestions I thought Robert might ask if he were around aswell as an array of lyrics from artists such as Larie Anderson, David Byrne, Daft Punk and Kraftwerk.  I then wrote a functionthat would allow Robert to compose lyrics in a song like manner (i.e, refrain, lyrics, hook) and console.log() the results.</p>
                    <br></br>
                        <h4>Web Speech API</h4>
                    <br></br>
                    <p>Robert still needed to sing. I rewrote those functions(the ones above to better support spoken word) and spent a number ofdays  experiementing with the Web Speech API in hopes that I could make Robert REALLY sing. I was hoping that I could use aPolySynth from  Tone.js or a sampler to create modulations and pitch changes in Robert's voice.  As both the Web Speech API andthe Web Audio (Tone is just building on top), I thought that feeding Robert's speechUtterance into a sampler or polysynth(or viceversa) might allow for some pitch modulation.  I am still working through thisd problem and as it stands, Robert "sings" hislyrics using a series of functions that allow for him to pick a random pitch and speed between each speech phrase (I don'tbelieve it is possible to chaing  Robert's speech during an utterance). Unfortunatley, this really doesn't do very much as far asmaking him sound like he's singing.If you know a secret, please email me as I really want to figure this one out (hamecow@gmailcom)!</p>
                    <br></br>
                    <p>BUT WAIT. THERE'S MORE.</p>
                    <br></br>
                        <h4>a frame</h4>
                    <p>An art obsessed cycloptic plant needs a beautiful environment to make its music, right? As the future progresses and we spendmore and more time at home, I decided that playing with A-Frame, an entity componenent system for Three.js (Dreams) allowing fornewbie HTML building of VR and AR scenes would be a fantastic learning opportunity.  In the very least, I have learned just howmuch I do not know and have to learn about 3D-Object modeling and how easy it is to lose a solid hour flipping an object acrossthe wrong axis over and over and over. I love it. And truly cannot wait to construct more of Robert's reality. I definitely needto learn how to better support my application and improve rendering speed and quality.  When things get 3D your files get lessSPEEDY.  On that note:</p>
                     <br></br>
